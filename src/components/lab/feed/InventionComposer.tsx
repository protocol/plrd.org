...
function Composer({onClose,bench,updateId,draftId}:Props&{bench:ReturnType<typeof useInventionBench>}){
 ...
 <form onSubmit={e=>{e.preventDefault();if(!active.current||busy)return;const result=bench.act({type:'save-update',update:value,requireExisting:!!updateId});setNotice(result.ok?'Text saved locally on My bench, together with selected media and captions. Media is not stored on a server; nothing was uploaded.':result.error||'Not saved.');if(result.ok){dirty.current=false;setSaved(true)}}}>
