...
function Following(){
 ...
 <h3>Followed disciplines</h3><ul>{f.prefs.disciplines.map(id=><li key={id}><span>{DISCIPLINES.find(d=>d.id===id)?.label||id}</span><button onClick={()=>f.act({type:'toggle',kind:'disciplines',id})}>Unfollow discipline</button></li>)}</ul>{!f.prefs.disciplines.length&&<p>No disciplines followed yet.</p>}
