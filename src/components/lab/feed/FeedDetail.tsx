...
function Detail({ row: originalRow, onClose }: { row: FeedRow; onClose: () => void }) {
  ...
  return <LabDialog title={row.title} onClose={onClose}><div className={styles.detail}>
    ...
    <details><summary>Curate tech-tree branch tags locally</summary><p>Connect this idea to more than one branch of the tech tree. Your tags shape your feed and saved views; they do not relabel a public source or publish a community taxonomy.</p><div className={styles.chips}>{fields.map(f => <button key={f.id} aria-pressed={tags.includes(f.id)} onClick={() => setTags(tags.includes(f.id) ? tags.filter(t => t !== f.id) : [...tags, f.id])}>{f.label}</button>)}</div><button onClick={() => { const result=following.act({type:'tag-idea',id:row.ideaId,disciplines:tags}); setNotice(result.ok ? 'Branch tags saved locally for this idea.' : result.error || 'Not saved.') }}>Save idea tags</button></details>
