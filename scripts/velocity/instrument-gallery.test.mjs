...
test('every instrument counts actual charts, including secondary, patent and sourced market probabilities', () => {
  ...
  for (const { id } of VELOCITY_INSTRUMENTS) {
    ...
    assert.equal(g.chartCount, id === 'markets' ? 5 : id === 'idea_vintage' ? 4 : 3, id)
...
test('idea vintage never borrows another area, even with no primary or unknown area', () => {
  const record = { instrument: 'idea_vintage', state: 'reading' }
  const examples = [{ label: 'Neurotech', series: line, scale: 'linear' }, { label: 'AI & Robotics', series: line, scale: 'linear' }]
  assert.equal(gallery(record, [], [], examples, 'Neurotech').chartCount, 1)
  assert.equal(gallery(record, [], [], examples, 'Unknown').chartCount, 0)
  assert.equal(gallery(record, [], [], examples).chartCount, 0)
})
...
test('Neuro galleries use declared instrument attribution and retain every augmentation exactly once', async () => {
  ...
  assert.deepEqual(groups.map(g => g.chartCount), [3, 1, 1, 1, 0])
