...
test('GET returns the versioned public contract for each area with only that field’s records and forecasts', async (t) => {
  ...
  for (const { key, label } of FOCUS_AREAS) {
    ...
    assert.deepEqual(Object.keys(body).sort(), ['schemaVersion', 'generatedAt', 'area', 'source', 'instruments', 'records', 'inflectionPoints', 'marketSignals', 'toolkit', 'methodology'].sort())
